...
class ObjectDetector:
    ...
    def __init__(self, model_size='small', conf_thres=0.25, iou_thres=0.45, classes=None, device=None):
        ...
        Args:
            model_size (str): Model size ('nano', 'small', 'medium', 'large', 'extra') or path to a .pt file
        ...
        # Map model size to model name, or use custom .pt file
        if model_size.lower().endswith('.pt'):
            model_name = model_size  # Use the .pt file directly
        else:
            model_map = {
                'nano': 'yolo11n',
                'small': 'yolo11s',
        ...
        # Load model
        try:
            self.model = YOLO(model_name, device=self.device)
            self.model.to(self.device)
            print(f"Loaded YOLO model {model_size} on {self.device}")
        ...
        if track:
            ...
            for predictions in results:
                ...
                for bbox in predictions.boxes:
                    ...
                    ids = [bbox.id] if hasattr(bbox, 'id') and bbox.id is not None else [None] * len(scores)
    ...
    def get_class_names(self):
        ...
        """
        return list(self.model.names.values()) if isinstance(self.model.names, dict) else self.model.names